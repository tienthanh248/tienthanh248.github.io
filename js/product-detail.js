...
document.querySelectorAll('.product-img-item').forEach(e => {
    e.addEventListener('click',()=> {
        ...
        // tra ve gia tri cua src voi ham getattribute
        
        document.querySelector('#product-img > img').setAttribute('src',img)
        // them thuoc tinh da chi dinh vao phan tu src , cung cap cho no gia tri la img
    })
})
document.querySelectorAll('.product-img-item').forEach(e => {
    e.addEventListener('click', () => {
...
document.querySelector('#view-all-description').addEventListener('click',()=>{
    ...
    // add them phan tu html , su dung toan tu 3 ngoi 
    document.querySelector('#view-all-description').innerHTML=content.classList.contains('active') ?
    'view less' : 'view all'
    /* contains tra ve 1 gia tri boolean xem 1 nut co phai con nut dk chi dinh k neu dungs
      add active thi hien ra view less, khong thi hien ra view all
     */
...
document.querySelector('#plus').addEventListener('click',()=> {
    var t = document.querySelector('#textbox').value ;
    document.querySelector('#textbox').value=parseInt(t)+1;
})
